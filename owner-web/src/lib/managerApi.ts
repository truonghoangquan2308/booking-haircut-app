...
export async function fetchManagerOrders(uid: string): Promise<ShopOrderRow[]> {
  const res = await fetch(`${getApiBase()}/api/manager/shop-orders`, {
    headers: headers(uid, false),
    cache: "no-store",
  });
  const data = await readJsonResponse<{
    orders?: ShopOrderRow[];
...
/** Thợ thuộc chi nhánh (Manager: users.branch_id; Owner: branches.owner_id + header Firebase). */
...
/** GET /api/manager/stats — doanh thu + lịch/đơn theo ngày (theo chi nhánh) */
