...
router.get('/products', async (_req, res) => {
  try {
    ...
    const validSortFields = ['id', 'sku', 'name', 'price', 'stock', 'created_at'];
    const sortField = validSortFields.includes(sort_by) ? sort_by : 'sku';
...
router.post('/admin/product-categories', async (req, res) => {
  try {
    const { name, code_prefix, description, is_active } = req.body ?? {};
...
router.post('/admin/products', handleProductUpload, async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      `
      INSERT INTO products (sku, category_id, name, description, price, stock, unit, image_url, is_active)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      `,
      [
        sku,
    ...
    const [productRows] = await pool.query(
      `
      SELECT id, sku, category_id, name, description, price, stock, unit,
...
router.put('/admin/product-categories/:id', async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      SET name = ?, code_prefix = ?, description = ?, is_active = ?
      WHERE id = ?
      `,
      [
        String(name).trim(),
