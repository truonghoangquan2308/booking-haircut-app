...
export async function createVnpayPayment(
  ...
  const res = await fetch(`${getApiBase()}/api/manager/vnpay/checkout`, {
    method: "POST",
    headers: headers(uid, true, branchId),
    body: JSON.stringify({ appointment_id: appointmentId }),
  });
  const data = await readJsonResponse<{ payment_url?: string; error?: string }>(res);
...
export async function fetchManagerCustomers(
  ...
  const res = await fetch(`${getApiBase()}/api/manager/customers`, {
    headers: headers(uid, false, branchId),
    cache: "no-store",
  });
  const data = await readJsonResponse<{
...
export async function createAppointmentOnBehalf(
  ...
  const res = await fetch(`${getApiBase()}/api/manager/appointments-on-behalf`, {
    method: "POST",
    headers: headers(uid, true, branchId),
    body: JSON.stringify(body),
  });
  ...
  if (!res.ok || !data.appointment_id) {
    throw new Error(data.error ?? "Tạo lịch thất bại");
  }
  return { appointment_id: data.appointment_id };
}

export async function fetchBarberTimeSlots(
  barberId: number,
  date: string,
  ...
  const data = await readJsonResponse<{
    slots?: Array<{ id: number; start_time: string; end_time: string; is_booked: number }>;
    error?: string;
  }>(res);
  if (!res.ok) throw new Error(data.error ?? "Lỗi tải khung giờ");
  return data.slots ?? [];
}
