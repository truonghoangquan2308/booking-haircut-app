...
export default function ReceptionistDashboardPage() {
  type TimeSlot = { id: number; start_time: string; end_time: string; is_booked: number };
  ...
  const [apptBarberId] = useState<number>(0);
  const [apptServiceId] = useState<number>(0);
  ...
  async function openVnpayForAppointment(appointmentId: number) {
    if (!uid) return;
    ...
    try {
      ...
      if (!paymentWindow) {
        throw new Error(
          "Trình duyệt chặn popup. Vui lòng cho phép mở tab mới để thanh toán.",
        );
      }
      const paymentUrl = await createVnpayPayment(uid, appointmentId, selectedBranchId ?? undefined);
  ...
  async function onAppointmentStatus(id: number, status: string) {
    ...
    try {
      ...
      if (status === "completed" && paymentWindow) {
        const paymentUrl = await createVnpayPayment(uid, id, selectedBranchId ?? undefined);
        paymentWindow.location.href = paymentUrl;
      }
    } catch (e) {
      if (paymentWindow && !paymentWindow.closed) {
            ...
            <div className="mt-6 flex flex-col gap-3 justify-end sm:flex-row">
              {selectedAppt?.status === "completed" && (
                <button
                  ...
                  onClick={() => void openVnpayForAppointment(selectedAppt.id)}
                >
                  {vnpayLoading ? "Đang tạo thanh toán..." : "Thanh toán VNPAY"}
                </button>
              )}
